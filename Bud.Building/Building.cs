...
namespace Bud {
    ...
    public static class Building {
        ...
        public static BuildTask Build(BuildAction action,
                                      string name = null,
        ...
        private static void LogBuildStart(TextWriter stdout,
                                          BuildTask buildTask,
                                          TaskNumberAssigner taskNumberAssigner,
                                          Stopwatch buildStopwatch,
                                          int thisTaskNumber) {
            var message = string.IsNullOrEmpty(buildTask.Name) ?
                              "Started building." :
                              $"Started building '{buildTask.Name}'.";
            WriteLogLine(stdout, thisTaskNumber, buildStopwatch, message, taskNumberAssigner.TotalTasks);
        }

        private static void LogBuildEnd(TextWriter stdout,
                                        BuildTask buildTask,
                                        TaskNumberAssigner taskNumberAssigner,
                                        Stopwatch buildStopwatch,
                                        int thisTaskNumber) {
            var message = string.IsNullOrEmpty(buildTask.Name) ?
                              "Done building." :
                              $"Done building '{buildTask.Name}'.";
            WriteLogLine(stdout, thisTaskNumber, buildStopwatch, message, taskNumberAssigner.TotalTasks);
        }

        private static void WriteLogLine(TextWriter logWriter,
                                         int taskNumber,
                                         Stopwatch stopwatch,
                                         string msg,
                                         int totalTasks)
